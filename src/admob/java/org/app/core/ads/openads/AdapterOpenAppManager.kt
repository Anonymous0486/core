...
class AdapterOpenAppManager private constructor() : LifecycleObserver,
    ...
    fun onStart() {
        ...
        Log.d(TAG, "Init in ON_START $adId - $isAdOtherClicked -> $isAdOtherShowFullScreen")
        ...
        Handler(Looper.getMainLooper()).postDelayed({
            if (isAdAvailable()) {
                if (currentActivity == null) {
                    ...
                    Log.i(TAG, "Show LaunchingOpenApp -->  $adId")
    ...
    fun showLoadingIfNeed(onCompleted: () -> Unit) {
        val rmAds = CoreRemoteConfig.instance.findAppOpenAds()
        Log.i(TAG, "showLoadingIfNeed: ${rmAds?.always_preload}")
        if (rmAds != null && rmAds.always_preload == true) {
            onCompleted.invoke()
        } else {
    ...
    private fun loadOpenAds() {
        ...
        if (!CoreAds.isNetworkAvailable(myApplication!!)) {
            Log.i(TAG, "Load ads network is not connected!!!")
            return
        }
        ...
        val loadCallback = object : AppOpenAdLoadCallback() {
            ...
            override fun onAdLoaded(ad: AppOpenAd) {
                Log.d(TAG, "onAdLoaded: $isOpenType")
                isLoadingAd = false
                retryAttempt = 0.0
            ...
            override fun onAdFailedToLoad(loadAdError: LoadAdError) {
                Log.d(TAG, "onAdFailedToLoad: $isOpenType  ${loadAdError.message}")
    ...
    private fun loadAndShowOpenAds() {
        ...
        if (myApplication == null || adId == null || adId!!.isEmpty() || isLoadingAd) {
            ...
            Log.i(TAG, "Can not show because of null or empty id")
            return
        }
    ...
    private fun isAdAvailable(): Boolean {
        Log.d(TAG, "check ads is available -> ${appOpenAd != null}")
    ...
    fun disableOpenAds() {
        //Temporary ignore
        Log.i(TAG, "disableOpenAds--> $adId")
        openAdsEnable = false
    }

    fun enableOpenAds() {
        Log.i(TAG, "enableOpenAds--> $adId")
        openAdsEnable = true
    }
