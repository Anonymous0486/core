                    ...
                    override fun onLoadSuccess() {
                        ...
                        if (!isTimeout) {
                            admobNativeAdsViewsStorage[adsId]?.nativeAds?.poll()?.let { nativeAd ->
                                nativeAd.showAdView(layoutAdId, activity, container)
                ...
                if (lastAds != null) {
                    ...
                    if (lastAnyAds != null) {
                        ...
                        if (hideInLoading) {
                            ...
                            Log.i(TAG, "NativeAdmob: child count -> ${container.childCount}")
                            if (container.childCount == 0) {
                                val shimmer = createShimmer(activity, layoutAdId)
                                shimmer.startShimmer()
                                container.removeAllViews()
...
//        configurationBuilder.setTagForChildDirectedTreatment(RequestConfiguration.TAG_FOR_CHILD_DIRECTED_TREATMENT_FALSE)
    ...
    fun ensureAdapterInitialized(
        context: Context,
        @DrawableRes ctaBackgroundResource: Int = R.drawable.bg_round_100,
    ) {
        if (!_initialized) {
            initAdsAdapter(
                context = context,
        ...
        if (nativeAd == null) {
            ...
            if (aView != null) {
                ...
                if (hideInLoading) {
                    ...
                    Log.i("NativeAdmob", "No ads -> add shimmer ${container.childCount == 0} for waiting ads load: $adsId")
                    if (container.childCount == 0) {
                        val shimmer = createShimmer(activity, layoutAdId)
                        shimmer.startShimmer()
                        container.removeAllViews()
            ...
            Log.i("NativeAdmob", "is on loading ${customNativeAd.isLoading}")
